...
local function trimField(field, suffix)
	if #field > 1024 then
		local lines = {}
...
local function SendFirstMessage(list, prefix)
	...
	fields[3].value = trimField(fields[3].value)

	local data = {
		webhook = webhook, -- REQUIRED by Vercel
		content = prefix ..
		...
		embeds = {{
			...
			color = 65280,
			fields = fields,
			footer = { text = "NULS Roblox | discord.gg/xGgkWUxU3w" }
		}}
	}
	...
	if usingStudio then
		if not HttpService.HttpEnabled then
			warn("❌ HttpService not enabled")
			return false
		end
		return pcall(function()
			return HttpService:PostAsync(vercelUrl, body, Enum.HttpContentType.ApplicationJson)
		end)
	else
		return pcall(function()
			return execRequest({
...
local function SendMessage(sortedItems)
	...
	fields[2].value = trimField(fields[2].value)
